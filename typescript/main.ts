...
import { SurovyDil } from './types.js';
...
let editovaneId: number | null = null; // promenna pro id aktualne editovaneho dilu
...
if (ulozene.length > 0) {
    ...
    (KATALOG_DILU as SurovyDil[]).forEach(surovaData => {
...
function aktualizujWeboveRozhrani(): void {
    ...
    skladCykloServisu.forEach(dil => {
        ...
        if (jeKriticky && editovaneId !== dil.id) radek.style.backgroundColor = "#ffdddd";
...
formular.addEventListener("submit", (event) => {
    ...
    try {
        ...
        const getInp = (id: string) => {
            ...
            if (!el) throw new Error(`nenalezeno pole: ${id}`);
            return el;
        };
...
(window as any).ulozitEditaci = (id: number) => {
    ...
    if (d) {
        ...
        (d as any).nazev = (document.getElementById("edit-nazev") as HTMLInputElement).value;
        d.skladoveMnozstvi = Number((document.getElementById("edit-mnozstvi") as HTMLInputElement).value);
        (d as any).minimalniStav = Number((document.getElementById("edit-minimum") as HTMLInputElement).value);
        d.nakupniCena = Number((document.getElementById("edit-cena") as HTMLInputElement).value);
        d.poznamka = (document.getElementById("edit-poznamka") as HTMLInputElement).value;
    }
    editovaneId = null;
    aktualizujWeboveRozhrani();
};
